...
Then I HTTP PUT to http://www.snipph.it/api/xyzw/linked_to/abcd
...
Then I HTTP PUT to http://www.snipph.it/api/xyzw/lost_link_to/abcd

Scenario: Pick up broadcast
When I have a device located at 40.123, 60.123
And I pick up a broadcast from a tag with id "abcd"
Then I HTTP PUT to http://www.snipph.it/api/device_at/40.123/60.123/found/abcd

Scenario: Ask server for lost tag (which has not yet been found)
When I have a device with id "xyzw" and a paired tag "abcd"
And I've lost the link to the tag
Then I poll HTTP GET http://www.snipph.it/api/lost_tags/abcd
And I get a 404

Scenario: Ask server for lost tag (which has been found)
When I have a device with id "xyzw" and a paired tag "abcd"
And I've lost the link to the tag
And somebody does an HTTP PUT to http://www.snipph.it/api/device_at/40.123/60.123/found/abcd
Then I poll HTTP GET http://www.snipph.it/api/lost_tags/abcd
And I get a 200
